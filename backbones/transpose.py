...
class TransPose(nn.Module):
    ...
    def __init__(self, block, layers, **kwargs):
        ...
        self.deconv_layers = self._make_deconv_layer(
            1,   # deconv layer number
            [d_model],  # deconv filters number
            [4],  # deconv kernels size
        )

        self.deconv_layers_t = self._make_deconv_layer(
            1,   # deconv layer number
            [d_model],  # deconv filters number
            [2],  # deconv kernels size
        )
        '''
        ...
        self.final_layer = nn.Sequential(
            ...
            nn.ConvTranspose2d(d_model//4, d_model//4, 2, 2),
            nn.BatchNorm2d(d_model//4),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        feature = x.permute(1, 2, 0).contiguous().view(bs, c, h, w)
        x = self.deconv_layers(feature)
        binary = self.final_layer(x)

        result = OrderedDict(binary=binary)

        y = self.deconv_layers_t(feature)
        thresh = self.final_layer_t(y)
    ...
    def step_function(self, x, y):
        return torch.reciprocal(1 + torch.exp(-10* (x - y)))
...
def get_pose_net(num_layers, is_train, **kwargs):
    #num_layers = cfg.MODEL.EXTRA.NUM_LAYERS
    block_class, layers = resnet_spec[num_layers]
    model = TransPose(block_class, layers, **kwargs)
